"""Heilbronn convex 14 grader.

Evaluates programs that place 14 points in 2D to maximize the minimum triangle
area (normalized by convex hull area) formed by any 3 of the 14 points.
The program file must define a run() function returning
np.ndarray of shape (14, 2) with point coordinates.
"""
...
class Grader(TaskGrader):
    """Grader for the Heilbronn convex problem (14 points).
...
def _run_evaluation(program_path: str, timeout: int, python_cmd: list[str]) -> dict:
    ...
    script = textwrap.dedent(f"""\
        ...
        # Min triangle area over all C(14,3) = 364 triples
