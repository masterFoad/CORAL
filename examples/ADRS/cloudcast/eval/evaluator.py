...
def validate_broadcast_topology(bc_t, source_node, terminal_nodes, num_partitions, G):
            ...
            for edge in partition_paths:
                ...
                if not G.has_edge(edge_src, edge_dst):
                    invalid_paths.append(
                        (dst, partition_id, f"edge {edge_src}->{edge_dst} not in graph")
                    )
                    path_valid = False
                    break
                ...
                if path_nodes[-1] != edge_src:
                    invalid_paths.append(
                        (
                            dst,
                            partition_id,
                            f"path discontinuity: expected {path_nodes[-1]}, got {edge_src}",
                        )
                    )
                    path_valid = False
                    break
            ...
            if path_valid and path_nodes[-1] != dst:
                invalid_paths.append(
                    (
                        dst,
                        partition_id,
                        f"path does not reach destination: ends at {path_nodes[-1]}, expected {dst}",
                    )
                )
    ...
    if actual_partitions != expected_total_partitions:
        return (
            False,
            f"Data loss detected: expected {expected_total_partitions} partitions, got {actual_partitions}",
        )
